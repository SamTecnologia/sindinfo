	...
	/* FINAL DA VALIDAÇÃO DO CNPJ DO NOME DO ARQUIVO */
			...
			/* SE EXISTIR */
				if($dr)
				{
					...
					//$novoarquivo = $cnpj.$ano1;
					$novoarquivo = $ano1.'_'.$cnpj;
					$destino .=$novoarquivo;
					...
					if(move_uploaded_file($arquivo['tmp_name'],$destino))
					{
						...
						/* SE EMPRESA CADASTRADA */
							...
							else
							{
								...
								$insereempresa = sprintf("INSERT INTO bl_csind_empresa(csind_emp_cnpj, csind_emp_razao, csind_emp_email ) VALUES ('$cnpj', '$razao_social','$email')");
								$result2 = mysql_query($insereempresa,$sindicato) or die(mysql_error()); // Insert no banco de dados
								/* FINAL DE INSERÇÃO */
								
								/* CONSULTA EMPRESA PELO CNPJ */
								mysql_select_db($database_sindicato,$sindicato); //Conexao com o banco
							...
							/* FIM SE EMPRESA NÃO CADASTRADA */
					}
				}
		        /* FIM SE DIRETÓRIO EXISTIR */
				else
				{
					/* **************************************************************** */
					/* PARAMETROS PARA CÓPIA DO ARQUIVO */
